...
import {
  View, Text, ScrollView, ActivityIndicator, TouchableOpacity, Image, Alert,
} from 'react-native';
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View className="items-center pt-10 pb-6 px-6">
          <TouchableOpacity onPress={handleChangeAvatar} disabled={uploadingAvatar}>
            <View
              className="w-24 h-24 rounded-full bg-brand-dark-2 items-center justify-center mb-3"
              style={{ borderWidth: 3, borderColor: '#8DC63F' }}
            >
              {uploadingAvatar ? (
                <ActivityIndicator color="#8DC63F" />
              ) : profile?.avatar_url ? (
            ...
            <View
              className="absolute bottom-3 right-0 w-7 h-7 rounded-full bg-brand-green items-center justify-center"
              style={{ borderWidth: 2, borderColor: '#1A1D1C' }}
            >
              <Ionicons name="camera" size={13} color="#1A1D1C" />
            </View>
          </TouchableOpacity>
          ...
          <Text className="text-brand-green text-sm font-semibold mt-0.5">
            {getLevelTitle(levelInfo.level)} · Nível {levelInfo.level}
          </Text>
        </View>
        ...
        <View className="mx-6 bg-brand-dark-2 rounded-2xl p-5 mb-4">
          ...
          <View className="flex-row justify-between mb-1">
            <Text className="text-gray-500 text-xs">Progresso para nível {levelInfo.level + 1}</Text>
          ...
          <View className="flex-row items-center justify-between mb-3">
            ...
            {myRank != null && (
              <View className="bg-brand-green/20 rounded-full px-2 py-0.5">
                <Text className="text-brand-green text-xs font-semibold">#{myRank} você</Text>
              </View>
            )}
          </View>
